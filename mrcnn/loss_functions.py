...
def smooth_l1_loss(y_true, y_pred):
    ...
    less_than_one = backend.cast(backend.less(diff, 1.0), "float32")
...
def rpn_class_loss_graph(rpn_match, rpn_class_logits):
    ...
    anchor_class = backend.cast(backend.equal(rpn_match, 1), tf.int32)
    # Positive and Negative anchors contribute to the loss,
    # but neutral anchors (match value = 0) don't.
    indices = tf.compat.v1.where(backend.not_equal(rpn_match, 0))
...
def rpn_bbox_loss_graph(config, target_bbox, rpn_match, rpn_bbox):
    ...
    rpn_match = backend.squeeze(rpn_match, -1)
    indices = tf.compat.v1.where(backend.equal(rpn_match, 1))

    # Pick bbox deltas that contribute to the loss
    rpn_bbox = tf.gather_nd(rpn_bbox, indices)

    # Trim target bounding box deltas to the same length as rpn_bbox.
    batch_counts = backend.sum(backend.cast(backend.equal(rpn_match, 1), tf.int32), axis=1)
...
def mrcnn_bbox_loss_graph(target_bbox, target_class_ids, pred_bbox):
    ...
    # Smooth-L1 Loss
    loss = backend.switch(tf.size(input=target_bbox) > 0,
...
def mrcnn_mask_loss_graph(target_masks, target_class_ids, pred_masks):
    ...
    target_masks = backend.reshape(target_masks, (-1, mask_shape[2], mask_shape[3]))
    pred_shape = tf.shape(input=pred_masks)
    pred_masks = backend.reshape(pred_masks,
    ...
    loss = backend.switch(tf.size(input=y_true) > 0,
                          backend.binary_crossentropy(target=y_true, output=y_pred),
                          tf.constant(0.0))
    loss = backend.mean(loss)
    return loss
