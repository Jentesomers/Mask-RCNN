...
def identity_block(input_tensor, kernel_size, filters, stage, block,
                   use_bias=True):                                          #Batch normalization has been deleted
    ...
    #bn_name_base = 'bn' + str(stage) + block + '_branch'       #Batch normalization has been deleted
...
def rpn_graph(feature_map, anchors_per_location, anchor_stride):
    ...
    rpn_bbox = layers.Lambda(lambda t: tf.reshape(t, [tf.shape(input=t)[0], -1, 4]))(x)
...
class MaskRCNN(object):
    ...
    def build(self, mode, config):
        ...
        if mode == "training":
            ...
            if not config.USE_RPN_ROIS:
                ...
                # Normalize coordinates
                target_rois = layers.Lambda(lambda x: norm_boxes_graph(
            ...
            model = models.Model([input_image, input_image_meta, input_anchors],
    ...
    def set_trainable(self, layer_regex, keras_model=None, indent=0, verbose=1):
        ...
        if verbose > 0 and keras_model is None:
            log.info("Selecting layers to train")
        ...
        for layer in layers:
            ...
            if trainable and verbose > 0:
                log.debug("{}{:20}   ({})".format(" " * indent, layer.name,
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers,
        ...
        log.info("Checkpoint Path: {}".format(self.checkpoint_path))
    ...
    def detect(self, images, verbose=0):
        ...
        if verbose:
            log.info("Processing {} images".format(len(images)))
            for image in images:
        ...
        if verbose:
            log.debug("molded_images", molded_images)
            log.debug("image_metas", image_metas)
    ...
    def detect_molded(self, molded_images, image_metas, verbose=0):
        ...
        if verbose:
            log.info("Processing {} images".format(len(molded_images)))
    ...
    def run_graph(self, images, outputs, image_metas=None):
        ...
        kf = backend.function(model.inputs, list(outputs.values()))
